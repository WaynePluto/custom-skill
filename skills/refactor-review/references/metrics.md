...
- 口径：连续整行注释超过 10 行即违规；行尾尾注不参与计数。豁免：用于生成文档的注解式注释块整块不计入——块内出现 `@` 开头的文档标签行（`@param`、`@Summary`、`@Router`、`@OA\*` 等），或块以 `/**`、`///` 开头（Javadoc/JSDoc/XML 文档注释）。这类注释是文档的数据源，不限制行数。
